...
class ilCompetenceRecommenderAllGUI
{
    ...
    public function executeCommand(): void
    {
        ...
        if (isset($viewmode) && ($cmd == 'eval' || $cmd == 'all')) {
            $cmd = $viewmode;
        }
    ...
    protected function showAll(string $viewmode = "list"): void
    {
        ...
        $view_control = $factory->viewControl()->mode($actions, $aria_label)->withActive(
            $this->lng->txt("ui_uihk_comprec_" . $viewmode)
        );
        ...
        $aria_label = "filter";
        $filter_view = $factory->viewControl()->mode($actions, $aria_label)->withActive(
            $this->lng->txt("ui_uihk_comprec_" . $filter)
        );
    ...
    private function setBar($competence, string $profile_id = ""): string
    {
        ...
        if ($competence["id"] == 0) {
            $id = $profile_id . "_" . $competence["base_skill"];
        } else {
            $id = $profile_id . "_" . $competence["id"];
        }
        ...
        if ($score > 0) {
            ...
            foreach ($competence["resources"] as $resource) {
                ...
                $link = $renderer->render(
                    $factory->link()->standard(ilObject::_lookupTitle($obj_id), ilLink::_getLink((int) $resource["id"]))
                );
            ...
            if (count($resourcearray) > 0) {
                $btpl->setVariable(
                    "NUMBEROFMATERIAL",
                    $this->lng->txt("ui_uihk_comprec_number_material") . ": " . count($resourcearray)
                );
            }
            if ($resourcearray != []) {
    ...
    private function getModalContent($skill_id, $tref_id, $base_skill_id): \ILIAS\UI\Component\Legacy\Legacy
    {
        ...
        // todo überprüfen
        $this->ctrl->saveParameter($this, (string) $skill_id);
        $this->ctrl->saveParameter($this, (string) $base_skill_id);
        ...
        $cur_basic_skill_id = ((int) $_POST["basic_skill_id"] > 0)
            ...
            : (((int) $_GET["basic_skill_id"] > 0)
                ...
                : key($options));
        if ($tref_id == 0) {
            $cur_basic_skill_id = $base_skill_id;
        }
