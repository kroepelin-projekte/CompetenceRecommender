    ...
    protected function showDashboard(): void
    {
        ...
        if ($competences == []) {
            ...
            if ($init_obj != array()) {
                ...
                $html = $this->lng->txt('ui_uihk_comprec_no_formationdata') . " " . $renderer->render($factory->button()->standard(
        ...
        $atpl = new ilTemplate("public/Customizing/global/plugins/Services/UIComponent/UserInterfaceHook/CompetenceRecommender/templates/tpl.comprecBarColumnTitle.html", true, true);
        ...
        foreach ($competences as $competence) {
            ...
            $btpl = new ilTemplate("public/Customizing/global/plugins/Services/UIComponent/UserInterfaceHook/CompetenceRecommender/templates/tpl.comprecBar.html", true, true);
            $btpl->setVariable("TITLE", $competence["title"]);
            $btpl->setVariable("DESCRIPTION", $competence["description"]);
            if ($competence["id"] == 0) {
                $id = $competence["base_skill"];
            } else {
                $id = $competence["id"];
            }
