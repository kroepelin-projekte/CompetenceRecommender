...
class ilCompetenceRecommenderAlgorithm
{
    ...
    /**
     * Default values used when a normalization would divide by zero
     * (e.g. goal level == 1 or a competence has only one level).
     */
    private static float $default_diff_value = 0.0;
    private static float $default_percentage_value = 0.0;
    ...
    protected static function getInstance(): ilCompetenceRecommenderAlgorithm
    {
        if (is_null(self::$instance)) {
            self::$instance = new ilCompetenceRecommenderAlgorithm();
        }
    ...
    public static function getNumberOfCompetencesForActivities(): int
    {
        ...
        foreach ($profiles as $profile) {
            if ($profile_settings->get("checked_profile_" . $profile->getId()) == $profile->getId()) {
                $result = $db->query("SELECT spl.level_id, spl.base_skill_id, spl.tref_id
									FROM skl_profile_level AS spl
									WHERE spl.profile_id = '" . $profile->getId() . "'");
    ...
    public static function getInitObjects(int $profile_id = -1): array
    {
        ...
        foreach ($profiles as $profile) {
            if ($profile_id == -1 || $profile_id == $profile->getId()) {
                $ref_id = $settings->get("init_obj_" . $profile->getId());
                if (is_numeric($ref_id)) {
                    $ref_ids[] = array("id" => $ref_id, "title" => $profile->getTitle());
                }
            }
        }
        return $ref_ids;
     ...
     * @return array
     */
    public static function getCompetencesToProfile($profile, array $skillsToSort = [], int $n = 0): array
    {
        $db = self::getDatabaseObj();
    ...
    private static function getSkillData(array $skill, array $skillsToSort, int $n): array
    {
        ...
        if ($skill["tref_id"] != 0) {
            ...
            $score = self::computeScore((string) $skill["base_skill_id"], true);
        }

        $goal_denom = (int) (($goal["nr"] ?? 0) - 1);
        $scale_denom = (int) ($levelcount - 1);
        $adjusted_score = ($score == 0) ? 0 : ((int) $score - 1);

        $percentage = ($goal_denom === 0)
            ? self::$default_percentage_value
            : ($adjusted_score / $goal_denom);

        $diff = ($score == 0)
            ? (($scale_denom === 0)
                ? self::$default_diff_value
                : (1 - ($goal_denom / $scale_denom)))
            : (($goal_denom === 0)
                ? self::$default_diff_value
                : ($adjusted_score / $goal_denom));
        if ($n == 0 || ($score != 0 && $score < $goal["nr"])) {
            ...
            } elseif (!isset($skillsToSort[$skill["tref_id"]])) {
                //set everything a step down (-1) for that the percentage of the lowest step is 0%
            ...
            } elseif ($goal["nr"] > $skillsToSort[$skill["tref_id"]]["goal"]) {
                // if several profiles with same skill take maximum
                $skillsToSort[$skill["tref_id"]]["goal"] = $goal["nr"] - 1;
                $skillsToSort[$skill["tref_id"]]["percentage"] = $percentage;
                $skillsToSort[$skill["tref_id"]]["diff"] = $diff;
            }
        }
        return $skillsToSort;
    }
